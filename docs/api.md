...
fast_mode 可选。true 表示只做 AST/规则分析，推荐 Burp 被动扫描流程使用。
mode      可选。"fast" 或 "full"。"full" 会启用 Secret 候选的 LLM 增强队列。
...
如果 `fast_mode=true`，会覆盖 `mode`，按 fast 模式分析。
...
`mode:"full"` 建议只用于用户主动触发的深度分析，因为 LLM 调用可能增加耗时和成本。
